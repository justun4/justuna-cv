...
export function initFingerprint(cvData) {
  ...
  magnifier.addEventListener('dblclick', (e) => {
    e.stopPropagation();
...
function openFingerprintScanner() {
  ...
  overlay.innerHTML = `
    ...
    <div class="fp-panel">
      <div class="fp-title">PARMAK IZI ESLESTIRME</div>
      <div class="fp-subtitle">Hedef parmak izini asagidaki adaylar arasinda bulun</div>
      <div class="fp-target-area">
        <div class="fp-label">HEDEF</div>
      ...
      <div class="fp-hint hidden">Ipucu: Desenlerin genel seklini karsilastirin</div>
    ...
    el.addEventListener('click', () => {
      ...
      if (seed === targetSeed) {
        ...
        if (projectItems.length > 0) {
          ...
          projectHTML = `<div class="fp-project"><strong>${p.title}</strong> (${p.tech}, ${p.year})<br>${p.description}</div>`;
        }

        resultEl.innerHTML = `
          <div class="fp-success-title">ESLESME BULUNDU</div>
          ${projectHTML}
